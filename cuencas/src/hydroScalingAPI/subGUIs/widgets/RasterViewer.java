...
public abstract class RasterViewer extends javax.swing.JInternalFrame {
    ...
    private void updateBasinsPopupMenu() {
        ...
        if (logFile.exists()) {
            ...
            clearLogs = new javax.swing.JMenuItem();
            clearLogs.setFont(new java.awt.Font("Dialog", 0, 10));
            clearLogs.setText("Clear Log");
            basinsViewerPopUp.add(clearLogs);
    ...
    private void initializeTextEditor() {
        System.out.println("Initialize Text Edit Called" );
        java.io.File originalFile = metaData.getLocationBinaryFile();
        final java.io.File logFile = new java.io.File(originalFile.getParent() + "/" + originalFile.getName().substring(0, originalFile.getName().lastIndexOf(".")) + ".log");
        frame = new JDialog();
    ...
    private void saveFile(File logFile) {
    ...
    private void addPreselectedBasinCheckBox(String basinLabel, boolean alreadySelected) {

        final javax.swing.JCheckBoxMenuItem showBasinI = new javax.swing.JCheckBoxMenuItem();
        showBasinI.setFont(new java.awt.Font("Dialog", 0, 10));
        showBasinI.setText(basinLabel);
        if (alreadySelected) {
            showBasinI.setSelected(true);
            basinsToDelete.add(basinLabel);
        }
        ...
        showBasinI.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ...
                if (showBasinI.isSelected()) {
                    ...
                    basinsToDelete.add(showOrderW.getText());
                } else {
                    System.out.println("Inside Else");
                    basinsToDelete.remove(showOrderW.getText());
                    removeBasinFromDisplay(showOrderW.getText());
    ...
    private void removeBasinFromDisplay(String BasinName) {

        try {

            if (divideBasinTable.containsKey(BasinName) && outletBasicTable.containsKey(BasinName)) {
                System.out.println("Deleting Valuyes of Basin");
                display.removeReference((visad.DataReferenceImpl) divideBasinTable.get(BasinName));
                display.removeReference((visad.DataReferenceImpl) outletBasicTable.get(BasinName));
                display.reDisplayAll();
                divideBasinTable.remove(BasinName);
                outletBasicTable.remove(BasinName);
    ...
    private void deletePreselectedBasin(ArrayList<String> basinLabel, java.awt.event.ActionEvent evt) throws IOException {
        ...
        if (dialogResult == 0) {
            ...
            ArrayList<String> oldList = new ArrayList<String>(Arrays.asList(localBasinsLog.getPresetBasins()));
            for (int i = 0; i < basinLabel.size(); i++) {
                if (oldList.contains(basinLabel.get(i))) {
                    oldList.remove(basinLabel.get(i));
                    removeBasinFromDisplay(basinLabel.get(i));
                }
            }

            logFile.delete();
            java.io.File newLogFile = new java.io.File(originalFile.getParent() + "/" + originalFile.getName().substring(0, originalFile.getName().lastIndexOf(".")) + ".log");

            try {
                FileWriter f2 = new FileWriter(newLogFile, false);
                for (int i = 0; i < oldList.size(); i++) {
                    f2.write(oldList.get(i));
                ...
                basinsViewerPopUp.removeAll();
                updateBasinsPopupMenu();
                // To reinitialize the vector with Latest values
                localBasinsLog = new BasinsLogReader(newLogFile);
            } catch (IOException e) {
    ...
    private void showBasinPreSelectedActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        this.currentBasinLabel = showOrderW.getText();
    ...
    /**
     * This method creates a dataReference for the polygon describing a basin
     * contour. The basin is interctively selected by the user
     *
     * @param MatX The column number of the basin outlet
     * @param MatY The row number of the basin outlet
     * @param isNew A boolean flag indicating if this is a previously selected
     * basin or a new one
        ...
        if (MatX >= 0 && MatY >= 0 && MatX < metaData.getNumCols() && MatY < metaData.getNumRows()) {
            if (isNew) {
                try {
                    ...
                    //Aniket
                    this.currentBasinLabel = basinLabel;
                    localBasinsLog.addBasinToFile(basinLabel);
                    //addPreselectedBasin(basinLabel);
                    addPreselectedBasinCheckBox(basinLabel, true);
                } catch (java.io.IOException exc) {
            ...
            //Aniket
            if (!divideBasinTable.containsKey(this.currentBasinLabel)) {
                divideBasinTable.put(this.currentBasinLabel, riverRef);
            }
            ...
            //Aniket
            if (!outletBasicTable.containsKey(this.currentBasinLabel)) {
                outletBasicTable.put(this.currentBasinLabel, rtref1);
            }
    ...
    /**
     * This method creates a dataReference for the line describing a river
     * pathway
     *
    ...
    protected void traceRiverPath(int MatX, int MatY) throws visad.VisADException, java.rmi.RemoteException {
        if (MatX >= 0 && MatY >= 0 && MatX < metaData.getNumCols() && MatY < metaData.getNumRows()) {
            ...
            display.addReference(riverRef, lineCMap);
            traceRiverDisplayRef.add(riverRef);
        }

    }

    //Aniket
    protected void removeTraceRiverReferences() {
        for (int i = 0; i < traceRiverDisplayRef.size(); i++) {
            try {
                display.removeReference((ThingReference) traceRiverDisplayRef.get(i));
            } catch (VisADException ex) {
                Logger.getLogger(RasterViewer.class.getName()).log(Level.SEVERE, null, ex);
            } catch (RemoteException ex) {
                Logger.getLogger(RasterViewer.class.getName()).log(Level.SEVERE, null, ex);
    ...
    protected void assignSubDataSet(int MatX, int MatY) throws visad.VisADException, java.rmi.RemoteException {
        if (MatX >= 0 && MatY >= 0 && MatX < metaData.getNumCols() && MatY < metaData.getNumRows()) {
            ...
            System.out.println("inside AssingDataset Value of X and Y " + MatX + " " + MatY);
    ...
    private void extractSubDataSet() {
        ...
        try {
            ...
            for (int j = iniY; j < iniY + lenY; j++) {
                for (int i = iniX; i < iniX + lenX; i++) {
    ...
    private void plotVectors() {
        ...
        for (int i = 0; i < actVectors.length; i++) {
            ...
            if (theCheckBox.isSelected()) {
                System.out.println("Value of Check Box" + theCheckBox.getText());
                ...
                if (theCheckBox.getText().lastIndexOf(".kml") != -1) {
                    if (mainFrame.getInfoManager().getDataReference("kml" + theCheckBox.getText()) == null) {
                        Runnable addVector = new Runnable() {
                            public void run() {
                                try {
                                    System.out.println("Insede KMl");
                                    double minX = metaData.getMinLon();
                                    double maxX = metaData.getMaxLon();
                                    double minY = metaData.getMinLat();
                                    double maxY = metaData.getMaxLat();

                                    ReadKML kml = new ReadKML(mainFrame.getInfoManager().dataBaseVectorsPath.getPath() + "/" + theCheckBox.getText());

                                    visad.UnionSet toPlot = kml.getUnionSet();
                                    System.out.println("Value of KML : " + toPlot.toString());
...
//                                       
     ...
     *
     * @param gaugesWithNames A boolean flag indicating if the gauge points must
     * have a label with the code
     * @param locationsWithNames A boolean flag indicating if the locations
     * points must have a label with the code
     */
    public void refreshReferences(final boolean gaugesWithNames, final boolean locationsWithNames) {
        ...
        plotField();
        if (showVectorLayer.isSelected()) {
            plotVectors();
        }
        if (showPolygonsLayer.isSelected()) {
            plotPolygons();
        }
    ...
    private void updateRelatedMaps() {
        ...
        for (int i = 0; i < mapNames.length; i++) {
            if ((mapNames[i].toString()).lastIndexOf(metaData.getName()) != -1) {
                currentMap = i;
            }
    ...
    private void initComponents() {
        ...
        visualToolbar.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        visualToolbar.setPreferredSize(new java.awt.Dimension(130, 22));

        restoreDisplay.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/Home16.gif"))); // NOI18N
        ...
        zoomInDisplay.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/ZoomIn16.gif"))); // NOI18N
        ...
        zoomOutDisplay.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/ZoomOut16.gif"))); // NOI18N
        ...
        snapshot.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/Snapshot.gif"))); // NOI18N
        ...
        print.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/Print.gif"))); // NOI18N
        ...
        showControls.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/ControlPanel.gif"))); // NOI18N
        ...
        demToolbar.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        demToolbar.setPreferredSize(new java.awt.Dimension(130, 22));

        showNetwork.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/Network.gif"))); // NOI18N
        ...
        showBasins.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/Predef_Outlet.gif"))); // NOI18N
        ...
        selectOutlet.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/Outlet.gif"))); // NOI18N
        ...
        selectOutlet.setSelectedIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/OutletActive.gif"))); // NOI18N
        ...
        traceStream.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/TraceRiver.gif"))); // NOI18N
        ...
        traceStream.setSelectedIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/TraceRiverActive.gif"))); // NOI18N
        ...
        locationToolbar.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        locationToolbar.setPreferredSize(new java.awt.Dimension(130, 22));

        createLocation.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/NewLocation.gif"))); // NOI18N
        ...
        createLocation.setSelectedIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/NewLocationActive.gif"))); // NOI18N
        ...
        cutSubDataSet.setIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/CutSubDem.gif"))); // NOI18N
        ...
        cutSubDataSet.setSelectedIcon(new javax.swing.ImageIcon(getClass().getResource("/hydroScalingAPI/subGUIs/configuration/icons/CutSubDemActive.gif"))); // NOI18N
        ...
        jButton1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                LocatePointClicked(evt);
            }
        });
        locationToolbar.add(jButton1);
        ...
        relatedMaps.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        ...
        showVectorLayer.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        ...
        showLinesLayer.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        ...
        showPolygonsLayer.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        ...
        showSitesLayer.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
    ...
    private void zoomInDisplayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_zoomInDisplayActionPerformed
        try {
            System.out.print("Zoom Performed");
            visad.ProjectionControl pc = display.getProjectionControl();
            double[] scaleMatrix = dr.getMouseBehavior().make_matrix(0.0, 0.0, 0.0,
                    2.0, 2.0, 2.0,
                    0.0, 0.0, 0.0);
    ...
    private void LocatePointClicked(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_LocatePointClicked

        /*  System.out.println("Inside Locate point");
         LocatePointView1 frame  =  new LocatePointView1(mainFrame, closable);
         frame.pack();
         frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
         frame.setBounds(100, 100, 600, 450);
         initializeDisplayForLPMethod(frame);
         frame.setVisible(true); */
      ...
      //  dialog.setModalityType(Dialog.ModalityType.TOOLKIT_MODAL);
        ...
        hydroScalingAPI.subGUIs.widgets.FixPointViewer thisDemViewer1 = null;
        try {

        

          /* hydroScalingAPI.io.MetaRaster sectionMR = new hydroScalingAPI.io.MetaRaster(metaData); 
            subSetCorners[0][0] = 150;
            subSetCorners[0][1] = 50;

            subSetCorners[1][0] = 90;
            subSetCorners[1][1] = 0;
          ...
          //   byte[][] basinMaskFullMap=myCuenca.getBasinMask();
           // byte [][] matDirs=new hydroScalingAPI.io.DataRaster(sectionMR).getByte(); */
            
           
           System.out.println( "No of Rows " + metaData.getNumRows() + " Value of Columns " + metaData.getNumCols());
                thisDemViewer1 = new hydroScalingAPI.subGUIs.widgets.FixPointViewer(mainFrame, metaData, relatedMapsList);
            }catch (VisADException ex) {
            Logger.getLogger(RasterViewer.class.getName()).log(Level.SEVERE, null, ex);
        }catch (IOException ex) {
            Logger.getLogger(RasterViewer.class.getName()).log(Level.SEVERE, null, ex);
        }
            dialog.add(thisDemViewer1);
            try {
                thisDemViewer1.setSelected(true);
    ...
    }//GEN-LAST:event_LocatePointClicked
